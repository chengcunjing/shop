<template>
  <header class="header">
    ...
    <div class="navbar">
      ...
      <div class="navbar-right-container" style="display: flex;">
        <div class="navbar-menu-container">
          ...
          <a href="javascript:void(0)" class="navbar-link" @click="loginclick" v-if="!nickName">登录</a>
          <a href="javascript:void(0)" class="navbar-link" v-if="nickName" @click="logout">退出登录</a>
    ...
    <div class="md-modal modal-msg md-modal-transition" :class="{'md-show':mdshowflag}">
      <div class="md-modal-inner">
        <div class="md-top">
          <div class="md-title">Login in</div>
          <button class="md-close" @click="cloceclick">Close</button>
        </div>
        <div class="md-content">
          <div class="confirm-tips">
            <div class="error-wrap">
              <span class="error" :class="{'error-show':errorshow}">用户名或者密码错误</span>
            </div>
            <ul>
              <li class="regi_form_input">
                <i class="icon IconPeople"></i>
                <input type="text" tabindex="1" name="loginname" placeholder="User Name" data-type="loginname"
                       class="regi_login_input regi_login_input_left" v-model="userName">
              </li>
              <li class="regi_form_input noMargin">
                <i class="icon IconPwd"></i>
                <input type="password" tabindex="2" name="password" placeholder="Password"
                       class="regi_login_input regi_login_input_left login-input-no input_text" v-model="userPwd">
              </li>
            </ul>
          </div>
          <div class="login-wrap"><a href="javascript:;" class="btn-login" @click="longin">登 录</a></div>
        </div>
      </div>
    </div>
    <div :class="{'md-overlay':mdoverlay}" @click="mdoverlayclick"></div>
  </header>
</template>
...
<script>
  ...
  export default {
    ...
    methods: {
      checkLogin() {
        axiox.get("/users/checkLogin").then(res => {
          if (res.data.status == "0") {
            // this.nickName = res.data.result
            this.$store.commit("setnickname", res.data.result);
            this.getcartcount()
          }
        })
      },
      ...
      longin() {
        ...
        axiox.post("/users/login", {
          ...
          if (res.data.status == "0") {
            ...
            this.$store.commit("setnickname", res.data.result.userName)
            this.getcartcount()
          } else {
            this.errorshow = true
          }
      ...
      logout() {
        axiox.post("/users/logout").then(res => {
          if (res.data.status == "0") {
            //this.nickName = ""
      ...
      getcartcount() {
        axiox.get("/users/getCartCount").then(res => {
          if (res.data.status == "0") {
