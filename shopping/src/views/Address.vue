<template>
  <div>
      ...
      <div class="container">
        <div class="checkout-addr">
          ...
          <div class="addr-list-wrap">
            <div class="addr-list">
              <ul>
                <li v-for="(item,index) in addressListFilter" :key="item.addressId" :class="{'check':checkIndex==index}"
                    @click="checkIndexclick(item,index);selectedAddrId=item.addressId">
                  <dl>
                    <dt>{{item.userName}}</dt>
                  ...
                  <div class="addr-opration addr-set-default">
                    <a href="javascript:;" class="addr-set-default-btn" v-if="!item.isDefault"
                       @click="setDefault(item.addressId)"><i>设为默认地址</i></a>
                  </div>
    ...
    <modal :mdShow="mdShow" @close="closeModal">
      ...
      <div slot="btnGroup">
        <a href="javascript:;" class="btn btn--m" @click="deladdressIdconfirm">确认</a>
        <a href="javascript:;" class="btn btn--m" @click="mdShow = false">取消</a>
      </div>
    </modal>
    <modal :mdShow="oenmdShow" @close="closeone">
      <p slot="message">
        最少保留一个收货地址
      </p>
      <div slot="btnGroup">
        <a href="javascript:;" class="btn btn--m" @click="oenmdShow = false">确认</a>
      </div>
    </modal>
